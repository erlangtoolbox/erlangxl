...
get_test() ->
    Args = mkargs(),
    ?assertEqual({[{a, "b"}],[]},
		strikead_yaws:params([strikead_yaws:get(a)], Args)).
...
convert_test() ->
    ...
    ?assertEqual({[{a, "b-x"}], []},
		strikead_yaws:params([
			strikead_yaws:convert(strikead_yaws:opt(a), Xf)], Args)).

any_test() ->
    Args = mkargs(),
    ?assertEqual({[{a, "b"}], []},
		strikead_yaws:params([strikead_yaws:any([
			strikead_yaws:get(x), strikead_yaws:opt(a)], "Not found")], Args)).
...
mkargs(Query) ->
    #arg{
        ...
        querydata = string:join(lists:map(fun({K,V}) ->
			atom_to_list(K) ++ "=" ++ V end, Query), "&")
    }.
