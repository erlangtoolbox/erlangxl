...
to_json(<<>>) -> "\"\"";
to_json(X) when is_binary(X) -> strikead_string:format("~p", [binary_to_list(X)]);
to_json(X) when is_number(X) -> strikead_string:format("~p", [X]);
to_json(true) -> "true";
to_json(false) -> "false";
to_json(undefined) -> "null";
to_json(X) when is_atom(X) -> strikead_string:format("\"~s\"", [X]);
...
ktuo_transform({obj, X}) ->
    [{strikead_string:to_atom(K), ktuo_transform(V)} || {K, V} <- X];
