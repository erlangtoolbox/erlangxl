...
nlist_filter(Pid, Mask) when is_list(Mask) ->
    nlist_filter(Pid, xl_file:compile_mask(Mask));
...
nlist_filter(Pid, Path, Mask) when is_list(Mask) ->
    nlist_filter(Pid, Path, xl_file:compile_mask(Mask));
...
download(Host, Username, Password, Dest, Path, Mask) when is_list(Mask) ->
    download(Host, Username, Password, Dest, Path, xl_file:compile_mask(Mask));
...
download(Pid, Dest, Path, Mask) when is_list(Mask) ->
    download(Pid, Dest, Path, xl_file:compile_mask(Mask));
...
ftp_error(E = {error, Code}, Target) ->
    case xl_io:is_posix_error(E) of
        true -> xl_io:posix_error(E, Target);
...
using(Host, Username, Password, F) -> xl_auto:using(?MODULE, [Host, Username, Password], F).
