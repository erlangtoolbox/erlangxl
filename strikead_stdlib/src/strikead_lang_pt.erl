-module(strikead_lang_pt).

-export([parse_transform/2, to_either/1]).
...
to_either({left, V}) -> {left, V};
to_either({right, V}) -> {right, V};
to_either({ok, V}) -> {right, V};
to_either(ok) -> {right, ok};
to_either(true) -> {right, true};
to_either(Left) -> {left, Left}.

%% do-notation lc transformation
transform_lc({lc, Ln, Expr, Generators}) -> transform_gen(Generators, Expr, Ln).

transform_gen([], Expr, LnE) -> [{tuple, LnE, [{atom, LnE, right}, expr(Expr)]}];
transform_gen([{generate,Ln, {var, _, VarName} = Var, GenF} | T], Expr, LnE) ->
    SynthVar = {var, Ln, list_to_atom("Either__" ++ atom_to_list(VarName))},
    LeftTuple = {tuple, Ln, [{atom, Ln, left}, Var]},
    [
        {match, Ln, SynthVar, {call, Ln, {remote, Ln, {atom, Ln, ?MODULE}, {atom, Ln, to_either}},[expr(GenF)]}},
        {'case', Ln, SynthVar, [
            {clause, Ln, [{tuple, Ln, [{atom, Ln, right}, Var]}], [], transform_gen(T, Expr, LnE) },
            {clause, Ln, [LeftTuple], [], [LeftTuple]}
        ]}
    ].

expr({call, Ln, {remote, Ln,{atom, Ln, strikead_lang}, {atom, Ln, do}}, [LC]}) -> {block, Ln, transform_lc(LC)};
