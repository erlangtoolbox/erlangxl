%% The contents of this file are subject to the Mozilla Public License
%% Version 1.1 (the "License"); you may not use this file except in
%% compliance with the License. You may obtain a copy of the License
%% at http://www.mozilla.org/MPL/
%%
%% Software distributed under the License is distributed on an "AS IS"
%% basis, WITHOUT WARRANTY OF ANY KIND, either express or implied. See
%% the License for the specific language governing rights and
%% limitations under the License.
%%
%% The Original Code is Erlando.
%%
%% The Initial Developer of the Original Code is VMware, Inc.
%% Copyright (c) 2011-2012 VMware, Inc; Eduard Sergeev.
%% All rights reserved.
%%

-module(do_tests).

-include_lib("eunit/include/eunit.hrl").

-compile({parse_transform, do}).

-compile(export_all).

sequence_test() ->
    List = lists:seq(1, 5),
    ListM = [do([option_m || return(N)]) || N <- List],
    ?assertEqual({ok, List}, monad:sequence(option_m, ListM)).

join_test() ->
    {ok, 5} = monad:join(option_m,
        option_m:return(option_m:return(5))),
    {ok, 5} = monad:join(option_m,
        do([option_m || return(option_m:return(5))])),
    {ok, 5} = monad:join(option_m,
        do([option_m || return(do([option_m || return(5)]))])).

maybe_test() ->
    undefined = option(atom),
    {ok, 9} = option(3).

option(Arg) ->
    do([option_m
        || monad_plus:guard(option_m, is_number(Arg)),
        return(Arg * Arg)]).

%% Classic recursive implementation of fibonnaci
fib_rec(N) when N >= 0 -> fib_rec(N, 0, 1).
fib_rec(0, _X, Y) -> Y;
fib_rec(N, X, Y) -> fib_rec(N - 1, Y, X + Y).

list_test() ->
    %% Demonstrate equivalence of list comprehensions and list monad
    A = [{X, Y} || X <- "abcd",
        Y <- [1, 2]],
    A = do([list_m || X <- "abcd",
        Y <- [1, 2],
        return({X, Y})]),
    %% Classic pythagorean triples
    P = [{X, Y, Z} || Z <- lists:seq(1, 20),
        X <- lists:seq(1, Z),
        Y <- lists:seq(X, Z),
        math:pow(X, 2) + math:pow(Y, 2) == math:pow(Z, 2)],
    P = do([list_m || Z <- lists:seq(1, 20),
        X <- lists:seq(1, Z),
        Y <- lists:seq(X, Z),
        monad_plus:guard(
            list_m, math:pow(X, 2) + math:pow(Y, 2) == math:pow(Z, 2)),
        return({X, Y, Z})]).

omega_test() ->
    A = [{X, Y, Z} || X <- "abcd",
        Y <- lists:seq(1, 5),
        Z <- lists:seq(11, 15)],
    B = do([omega_m || X <- "abcd",
        Y <- lists:seq(1, 5),
        Z <- lists:seq(11, 15),
        return({X, Y, Z})]),
    true = A =/= B,
    true = A =:= lists:usort(B).

%% Tests for 'let-match binding' (a-la 'let' in Haskell's 'do'
%% expression) But instead of 'let' here we use 'match' (=) expression
%% in 'do([])':
let_match_test() ->
    T1 = do([option_m ||
        R <- return(2),
        R2 = R * R,
        return(R2 * R2)
    ]),
    T1 = do([option_m ||
        R <- return(2),
        return(R * R * R * R)
    ]),
    %% Failure test
    T2 = do([option_m ||
        A <- return(42),
        {B, C} <- fail(test),
        BC = B * C,
        return(BC + A)
    ]),
    T2 = do([option_m ||
        A <- return(42),
        {B, C} <- fail(test),
        return(B * C + A)
    ]),

    Fun = fun({X, Y}) -> {Y, X} end, %% Mysterious function
    T3 = do([error_m || R <- return({1, 42}),
        {R1, R2} = Fun(R),
        return(R1 + R2)]),
    T3 = do([error_m || R <- return({1, 42}),
        %% No better way without 'let'?
        %% Well, only via extra 'return'
        return(element(1, Fun(R)) + element(2, Fun(R)))]),

    DivRem = fun(N, M) -> {N div M, N rem M} end,
    T4 = do([error_m || {N, M} <- return({42, 3}),
        {D, R} = DivRem(N, M),
        E <- T3,
        S = D + R + E,
        return({D, R, S})]),
    T4 = do([error_m || {N, M} <- return({42, 3}),
        %% Can hack it with extra 'return' (and '>>='
        %% as result)
        {D, R} <- return(DivRem(N, M)),
        E <- T3,
        return({D, R, D + R + E})]),

    T5 = do([list_m || X <- [1, 2, 3],
        X2 = X * X,
        Y <- lists:seq(1, X2),
        Y2 = {Y, X2},
        Z = Y + X2,
        return({X2, Y, Y2, Z})]),
    T5 = do([list_m || X <- [1, 2, 3],
        Y <- lists:seq(1, X * X),
        return({X * X, Y, {Y, X * X}, Y + X * X})]).

let_first_test() ->
    M = do([list_m ||
        A = 3,
        X <- [1, 2, A],
        Y <- [A, A + 1],
        return({X, Y})
    ]),
    M = fun() ->
        A = 3,
        do([list_m ||
            X <- [1, 2, A],
            Y <- [A, A + 1],
            return({X, Y})
        ])
    end().

let_escapes_test() ->
    M1 = do([option_m ||
        A = 5,
        return(A)]),
    M2 = do([option_m ||
        A = 6,
        return(A)]),
    M1 = do([option_m || return(5)]),
    M2 = do([option_m || return(6)]),

    %% Demonstrate that bindings do not escape.
    M3 = do([option_m || return(_A = 5)]),
    M3 = do([option_m || return((_A = 7) - 2)]),
    _A = 6.

f() ->
    [X || Z <- [1, 2],
        X <- Z].